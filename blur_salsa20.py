...
def ip_encrypt(str_to_be_enc,key,subnet):
	...
	hashed_key=hashlib.sha512(key).hexdigest()
	cipher=xsalsa20.XSalsa20(key=hashed_key[-32:],iv=hashed_key[-56:-32])
	ip_addr = []
	enc_ip_addr=''
	i=0
	digits = re.compile('\d+')
	...
	enc_ip_bytes = cipher.process(ip_bytes)
...
def ip_decrypt(str_to_be_dec, key, subnet):
	...
	for match in iterator:
		...
		i+=8
	ip_bytes = "".join(map(chr, ip_addr))
	dec_ip_bytes = cipher.process(ip_bytes)
...
def my_encrypt(text, key):	#return encrypted text string
	hashed_key=hashlib.sha512(key).hexdigest()
	cipher=xsalsa20.XSalsa20(key=hashed_key[-32:],iv=hashed_key[-56:-32])
	return base64.b64encode(cipher.process(text))

def my_decrypt(enc,key):
	hashed_key=hashlib.sha512(key).hexdigest()
	cipher=xsalsa20.XSalsa20(key=hashed_key[-32:],iv=hashed_key[-56:-32])
	return base64.b64encode(cipher.process(enc))
